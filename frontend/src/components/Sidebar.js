...
const Sidebar = () => {
  ...
  if (!currentDiagram) {
    return (
      <div className={`bg-dark-800/90 backdrop-blur-glass border-r border-neon-green/20 relative overflow-hidden flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-72'}`}>
        {/* Collapse Button */}
        <button
          onClick={toggleCollapse}
          className="absolute -right-3 top-6 z-20 bg-dark-700 border border-neon-green/30 rounded-full p-1 text-neon-green hover:bg-dark-600 transition-colors"
        ...
        <div className="relative z-10 p-4 flex flex-col items-center">
          <h3 className={`text-xl font-bold bg-gradient-to-r from-neon-green to-neon-cyan bg-clip-text text-transparent mb-4 ${isCollapsed ? 'hidden' : 'block'}`}>
            Service Catalog
          </h3>
          <p className={`text-slate-400 text-sm bg-dark-700/50 p-3 rounded-xl border border-neon-green/10 ${isCollapsed ? 'hidden' : 'block'}`}>
  ...
  return (
    <div className={`bg-dark-800/90 backdrop-blur-glass border-r border-neon-green/20 p-6 relative overflow-hidden flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-72'}`}>
      {/* Collapse Button */}
      <button
        onClick={toggleCollapse}
        className="absolute -right-3 top-6 z-20 bg-dark-700 border border-neon-green/30 rounded-full p-1 text-neon-green hover:bg-dark-600 transition-colors"
